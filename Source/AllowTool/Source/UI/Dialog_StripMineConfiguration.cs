...
namespace AllowTool
{
    public class Dialog_StripMineConfiguration : Window
    {
        ...
        {
            get => new Vector2(windowRect.x, windowRect.y);
            set
            {
        ...
        public override Vector2 InitialSize => new Vector2(320f, Margin * 2 + RowHeight * 5 + Spacing * 5);
        ...
        public override void DoWindowContents(Rect inRect)
        {
            ...
            if (Widgets.ButtonText(cancelBtnRect, "CancelButton".Translate())) CancelAndClose();
            var confirmBtnRect = buttonsRect.RightPartPixels(buttonsRect.width - cancelBtnRect.width - Spacing);
            GUI.color = Color.green;
            if (Widgets.ButtonText(confirmBtnRect, "AcceptButton".Translate())) AcceptAndClose();
            GUI.color = Color.white;
            if (settingsChanged) SettingsChanged?.Invoke(settings);
        ...
        private int DoIntSpinner(string label, int value, Listing_Standard listing, ref bool changed)
        {
            ...
            if (Widgets.ButtonText(spinnerRect.LeftPart(.333f), "-")) TryChangeValue(-1, ref changed);
            if (Widgets.ButtonText(spinnerRect.RightPart(.333f), "+")) TryChangeValue(1, ref changed);
            Text.Anchor = originalAnchor;
            return value;
        }
