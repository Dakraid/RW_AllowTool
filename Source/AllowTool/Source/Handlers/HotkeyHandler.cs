...
namespace AllowTool
{
    ...
    public class HotKeyHandler
    {
        ...
        public void OnGUI()
        {
            if (Event.current.type == EventType.KeyDown) CheckForHotkeyPresses();
        }
        ...
        public void RebindAllDesignators()
        {
            ...
            foreach (var designator in providers) activeListeners.Add(new HotkeyListener(designator, ((IGlobalHotKeyProvider) designator).GlobalHotKey));
        }

        private void CheckForHotkeyPresses()
        {
            if (Find.CurrentMap == null || Event.current.keyCode == KeyCode.None) return;
            if (AllowToolDefOf.ToolContextMenuAction.JustPressed) DesignatorContextMenuController.ProcessContextActionHotkeyPress();
            if (!AllowToolController.Instance.Handles.GlobalHotkeysSetting) return;
            for (var i = 0; i < activeListeners.Count; i++)
