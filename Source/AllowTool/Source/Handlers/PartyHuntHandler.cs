    ...
    public static class PartyHuntHandler
    {
                ...
                && !hunter.stances.FullBodyBusy)
            {
                ...
                if (hunter.drafter.FireAtWill)
                {
                    ...
                    if (target != null)
                    {
                        ...
                        if (target.Dead && WorldSettings.UnforbidDrops) target.Corpse?.SetForbidden(false, false);
                    }
                }
                                                 ...
                                                 && CanDoCommonerWork(hunter) && !AnyHuntingPartyMembersInCombat(hunter, MaxPartyMemberDistance))
                {
                    ...
                    if (target != null) target.ToggleDesignation(AllowToolDefOf.FinishOffDesignation, true);
                    ...
                    if (job != null)
                    {
                        ...
                        if (hunter.jobs.curDriver is JobDriver_FinishOff jdfo && WorldSettings.UnforbidDrops) jdfo.UnforbidBody = true;
        ...
        private static Pawn TryFindHuntingTarget(Pawn searcher, float minDistance, float maxDistance, HuntingTargetFilter targetFilter)
        {
            ...
            bool validator(Pawn pawn)
            {
                ...
                if (pawn.Position.Fogged(searcher.Map) || !searcher.CanSee(pawn)) return false;
            ...
            {
                var pawn = mapPawns[i];
                if (validator(pawn)) huntingTargetCandidates.Add(new HuntingTargetCandidate(pawn, (searcher.Position - pawn.Position).LengthHorizontalSquared));
            }
