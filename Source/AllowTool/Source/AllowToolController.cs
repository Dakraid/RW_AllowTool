...
namespace AllowTool
{
    ...
    public class AllowToolController : ModBase
    {
        ...
        public override void MapLoaded(Map map)
        {
            ...
            if (!Handles.HaulWorktypeSetting) AllowToolUtility.EnsureAllColonistsHaveWorkTypeEnabled(AllowToolDefOf.HaulingUrgent, map);
            if (!Handles.FinishOffWorktypeSetting) AllowToolUtility.EnsureAllColonistsHaveWorkTypeEnabled(AllowToolDefOf.FinishingOff, map);
        }
        ...
        public override void SettingsChanged()
        {
            ...
            if (AllowToolUtility.ReverseDesignatorDatabaseInitialized) Find.ReverseDesignatorDatabase.Reinit();
        }
        ...
        internal void OnBeforeImpliedDefGeneration()
        {
            try
            {
                ...
                if (!Handles.HaulWorktypeSetting) AllowToolDefOf.HaulingUrgent.visible = false;
                if (Handles.FinishOffWorktypeSetting) AllowToolDefOf.FinishingOff.visible = true;
            }
            catch (Exception e)
            {
        ...
        {
            foreach (var categoryDef in DefDatabase<DesignationCategoryDef>.AllDefs) Reflection.DesignationCategoryDefResolveDesignatorsMethod.Invoke(categoryDef, new object[0]);
        }
    }
}