...
namespace AllowTool
{
    ...
    public class Designator_SelectSimilar : Designator_SelectableThings
    {
        ...
        private bool SelectingSingleCell => Dragger.SelectionInProgress && Dragger.SelectedArea.Area == 1;
        ...
        public void ReindexSelectionConstraints()
        {
            try
            {
                ...
                foreach (var selectedObject in selector.SelectedObjects)
                {
                    ...
                    var constraintHash = GetConstraintHashForThing(thing);
                ...
                for (var i = 0; i < constraintList.Count; i++)
                {
                    ...
                    if (i < MaxNumListedConstraints - 1 || isLastEntry)
                    {
                        ...
                        if (constraint.stuffDef?.label != null) builder.AppendFormat(" ({0})", constraint.stuffDef.label.CapitalizeFirst());
