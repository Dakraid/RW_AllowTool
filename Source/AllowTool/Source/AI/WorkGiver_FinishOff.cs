...
namespace AllowTool
{
    ...
    public class WorkGiver_FinishOff : WorkGiver_Scanner
    {
        ...
        private static bool WorkGiverEnabled =>
            AllowToolController.Instance.Handles.IsDesignatorEnabled(AllowToolDefOf.FinishOffDesignator)
        ...
        public static FloatMenuOption InjectThingFloatOptionIfNeeded(Thing target, Pawn selPawn)
        {
            if (Designator_FinishOff.IsValidDesignationTarget(target))
                if (WorkGiverEnabled)
                {
                    ...
                    {
                        opt.Disabled = true;
                        if (JobFailReason.HaveReason) opt.Label = "CannotGenericWork".Translate(giver.def.verb, target.LabelShort, target) + " (" + JobFailReason.Reason + ")";
                    }
        ...
        {
            if (WorkGiverEnabled) return GetPotentialTargets(pawn);
        ...
        private IEnumerable<Thing> GetPotentialTargets(Pawn pawn)
        {
            ...
            {
                var target = pawns[i];
                if (Designator_FinishOff.IsValidDesignationTarget(target)) yield return target;
            }
        }
    }
}