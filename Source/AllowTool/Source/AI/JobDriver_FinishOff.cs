...
namespace AllowTool
{
    ...
    public class JobDriver_FinishOff : JobDriver
    {
        ...
        {
            if (job.playerForced) TargetA.Thing.ToggleDesignation(AllowToolDefOf.FinishOffDesignation, true);
            ...
            yield return new Toil
            {
                initAction = () =>
                {
                    ...
                    if (skullMote != null && !skullMote.Destroyed) skullMote.Destroy();
                    ...
                    if (!job.playerForced)
                    {
                        ...
                        if (opportunityJob != null) pawn.jobs.jobQueue.EnqueueFirst(opportunityJob);
                    }
                },
        ...
        private void DoExecution(Pawn slayer, Pawn victim)
        {
            ...
            for (var i = 0; i < num; i++) victim.health.DropBloodFilth();
            var part = victim.RaceProps.body.GetPartsWithTag(BodyPartTagDefOf.ConsciousnessSource).FirstOrDefault();
            var amount = part != null ? Mathf.Clamp((int) victim.health.hediffSet.GetPartHealth(part) - 1, 1, 20) : 20;
            var damageInfo = new DamageInfo(DamageDefOf.ExecutionCut, amount, -1f, -1F, slayer, part);
            victim.TakeDamage(damageInfo);
            if (!victim.Dead) victim.Kill(damageInfo);
            if (UnforbidBody) victim.Corpse?.SetForbidden(false, false);
            if (AllowToolController.Instance.Handles.FinishOffUnforbidsSetting) UnforbidAdjacentThingsTo(position, Map);
        }
        ...
        private void DoSocialImpact(Pawn victim)
        {
            ...
            if (giveThought) ThoughtUtility.GiveThoughtsForPawnExecuted(victim, PawnExecutionKind.GenericBrutal);
            if (victim.RaceProps != null && victim.RaceProps.intelligence == Intelligence.Animal) pawn.records.Increment(RecordDefOf.AnimalsSlaughtered);
            if (isPrisoner) TaleRecorder.RecordTale(TaleDefOf.ExecutedPrisoner, pawn, victim);
        }
        ...
        private void UnforbidAdjacentThingsTo(IntVec3 center, Map map)
        {
            foreach (var offset in GenAdj.AdjacentCellsAndInside)
            {
                ...
                if (pos.InBounds(map)) AllowToolUtility.ToggleForbiddenInCell(pos, map, false);
            }
        }
    }
}