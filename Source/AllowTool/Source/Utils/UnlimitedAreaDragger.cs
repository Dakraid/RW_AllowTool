...
namespace AllowTool
{
    ...
    public class UnlimitedAreaDragger
    {
        ...
        private void Update()
        {
            ...
            if (listening && map == null || Find.MapUI.designatorManager.SelectedDesignator != owningDesignator) StopListening();
            if (!listening) return;
            RegisterForNextUpdate();
            var dragger = Find.MapUI.designatorManager.Dragger;
            if (!SelectionInProgress && dragger.Dragging)
                OnSelectionStarted();
            else if (SelectionInProgress && !dragger.Dragging) OnSelectionEnded();
