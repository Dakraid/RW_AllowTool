    ...
    public static class DesignatorContextMenuController
    {
        ...
        public static void RebindAllContextMenus()
        {
            try
            {
                ...
                foreach (var designator in allDesignators) // check if designator matches the type required by any of the handlers
        ...
        public static void DrawCommandOverlayIfNeeded(Command command, Vector2 topLeft)
        {
            ...
            if (designator != null)
                try
                {
                    ...
                    if (designatorMenuProviders.ContainsKey(designator)) AllowToolUtility.DrawRightClickIcon(topLeft.x + overlayIconOffset.x, topLeft.y + overlayIconOffset.y);
                }
                catch (Exception e)
                {
        ...
        public static Designator TryResolveCommandToDesignator(Command command)
        {
            if (command != null)
            {
                ...
                if (designator != null) return designator;
                // for reverse designators
                if (currentDrawnReverseDesignators.TryGetValue(command, out designator)) return designator;
            }
        ...
        public static void ProcessContextActionHotkeyPress()
        {
            ...
            if (selectedDesignator != null)
            {
                ...
                if (!designatorMenuProviders.TryGetValue(selectedDesignator, out var provider)) provider = GetMenuProviderForDesignator(selectedDesignator);
            ...
            else if (AllowToolController.Instance.Handles.ExtendedContextActionSetting.Value)
            {
                ...
                foreach (var reverseDesignator in currentDrawnReverseDesignators.Values)
                    if (designatorMenuProviders.TryGetValue(reverseDesignator, out var reverseProvider))
        ...
        {
            ClearReverseDesignatorPairs();
            foreach (var reverseDesignator in AllowToolUtility.EnumerateReverseDesignators()) TryBindDesignatorToProvider(reverseDesignator);
        }
        ...
        {
            if (designator == null || designatorMenuProviders.ContainsKey(designator)) return;
            var provider = GetMenuProviderForDesignator(designator);
            if (provider.HasCustomEnabledEntries || DesignatorShouldHaveFallbackContextMenuProvider(designator)) designatorMenuProviders.Add(designator, provider);
        }

        private static ContextMenuProvider GetMenuProviderForDesignator(Designator designator)
        {
            for (var i = 0; i < menuProviders.Length; i++)
