        ...
        public override ActivationResult Activate(Designator designator, Map map)
        {
            ...
            foreach (var thing in map.listerThings.ThingsInGroup(DesignationRequestGroup))
                if (ThingIsValidForDesignation(thing) && designator.CanDesignateThing(thing).Accepted)
                {
                    ...
                    hitCount++;
                    if (AllowToolUtility.PawnIsFriendly(thing)) friendliesFound = true;
                }

            if (hitCount > 0 && friendliesFound) Messages.Message("Designator_context_finish_allies".Translate(hitCount), MessageTypeDefOf.CautionInput);
